...
<body>
    ...
    <?php if($info):?>
        ...
        <?php if($type == 'event'):?>
        <div style="font-size:11px;">Time: <?= date('h:i A',strtotime($info['start_time']));?> - <?=  date('h:i A',strtotime($info['end_time']));?> &nbsp; &nbsp;Subject: <?= $info['subj_name'];?> &nbsp;&nbsp; Professor: <?= $info['last_name'];?>, <?= $info['first_name'];?> <?= $info['m_initial'];?>&nbsp;&nbsp;&nbsp;&nbsp; Course: <?= $info['course_abbrev'];?> &nbsp;&nbsp;&nbsp;Year & Section: <?= $info['year'];?> - <?= $info['section'];?> &nbsp;&nbsp; S.Y: <?= $info['start_sy'];?>-<?= $info['end_sy'];?>  &nbsp;&nbsp; Sem: <?= $info['sem'];?> </div>
        <span style="font-size:11px;">Printed On: <?= date('F d, Y');?></span>
        <?php else:?>
        <span style="font-size:11px">Time: <?=  date('h:i A',strtotime($info['start_time']));?> - <?=  date('h:i A',strtotime($info['end_time']));?> &nbsp;&nbsp;&nbsp; Event Name: <?= $info['event_name'];?> &nbsp;&nbsp;&nbsp;&nbsp; Assigned Person: <?= $info['assigned_person'];?>&nbsp;&nbsp;&nbsp; Laboratory: <?= $info['lab_name'];?> &nbsp;&nbsp;&nbsp;&nbsp; Category: <?= $info['category'];?> &nbsp;&nbsp;&nbsp;&nbsp;Printed On: <?= date('F d, Y');?></span>
