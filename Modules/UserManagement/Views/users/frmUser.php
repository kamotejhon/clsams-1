                ...
                <div class="row">
                  <div class="col-md-6">
                    ...
                    <div class="input-group" id="section" >
                      ...
                      <select name="section_id" id="section_id" class="form-control">
                        ...
                        <option value="<?= $section['id'] ?>" <?= !empty($rec['section_id']) ?  ($section['id'] == $rec['section_id'] ) ? 'selected':'':'' ?>><?= ucwords($section['year']) ?> - <?= ucwords($section['section']) ?></option>
                        <?php endforeach; ?>
                      <!--  -->
                      </select>
                    </div>
                  ...
                  <div class="col-md-6">
                    <div class="form-group">
                      ...
                      <?php if($role['id'] !== '1'): ?>
                        <option value="<?= $role['id'] ?>" <?= isset($rec['role_id']) ? ($role['id'] == $rec['role_id'] ? 'selected':'') : ''?>><?= ucwords($role['role_name']) ?></option>
                      <?php endif;?>
                      <?php endforeach; ?>
                    </select>
                    ...
                    <div class="input-group" id="course" >
                      ...
                      <select name="course_id" id="course_id" class="form-control">
                        ...
                        <option value="<?= $course['id'] ?>" <?=   isset($rec['course_id']) ? (($course['id'] == $rec['course_id']) ? 'selected':''):'' ?>><?= ucwords($course['course_abbrev']) ?> - <?= ucwords($course['course_name']) ?></option>
                        <?php endforeach; ?>
                      </select>
                    </div>
...
<script src="<?=base_url();?>/plugins/inputmask/inputmask.min.js"></script>
  ...
  <script type="text/javascript">
    $(function(){
      ...
      $('#course').hide();
      $('#section').hide();
      var course_id = '<?= isset($rec['course_id']) ?? '' ?>';
      var section_id = '<?= isset($rec['section_id']) ?? ''?>';
