...
class Visitors extends BaseController
{
    ...
    public function index($offset = 0)
    {
		...
		if($_POST){
			...
			$data['visitors'] = $model->getVisitorsLabByDateAttendee(null,null);
			$data['rec'] = $_POST;
		}
	...
	public function login_visitor()
    {
    	...
    	if(!empty($_POST))
    	{
				if(empty($visit)){
					...
					$schedlab = $schedLabsmodel->getScheduleLabById($event_id);
					$visitor_total = count($model->getVisitorsLabById($event_id));
