...
class Labs extends BaseController {
    ...
    if($this->request->getMethod() === 'post'){
      if($this->validate('labs')){
        if($labsModel->add($_POST)){
          ...
          $this->session->setFlashData('success_message', 'Sucessfuly created a laboratory');
        } else {
    ...
    if($this->request->getMethod() === 'post'){
      if($this->validate('labs')){
        if($this->labsModel->edit($id, $_POST)){
          ...
          $this->session->setFlashData('success_message', 'Sucessfuly edited a laboratory');
        } else {
          $this->session->setFlashData('error_message', 'Something went wrong!');
        }
        return redirect()->to(base_url('admin/labs'));
  ...
  public function delete($id){
    ...
    if($labsModel->inactive($id)) {
      ...
      $this->session->setFlashData('success_message', 'Successfully deleted laboratory');
    } else {
      $this->session->setFlashData('error_message', 'Something went wrong!');
    }
    return redirect()->to(base_url('admin/labs'));
  ...
  public function active($id){
    ...
    if($labsModel->active($id)) {
      ...
      $this->session->setFlashData('success_message', 'Successfully restored laboratory');
    } else {
      $this->session->setFlashData('error_message', 'Something went wrong!');
    }
    return redirect()->to(base_url('admin/labs'));
