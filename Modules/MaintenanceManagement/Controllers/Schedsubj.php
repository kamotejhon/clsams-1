...
class Schedsubj extends BaseController {
  ...
  public function add(){
    ...
    if($this->request->getMethod() === 'post'){
      if($this->validate('schedsubj')){
        ...
        $input_date = isset($_POST['date']) ? date('Y-m-d', strtotime($_POST['date'])): [];
    
        if(in_array($input_date, $holi )){
          ...
          if($_POST['day']){
            ...
            if($schedsubj->add_schedsubj($_POST)){
              $this->activityLogsModel->addLogs($_SESSION['uid'], 'Add Sched Subject', 'admin/schedsubject', json_encode($_POST));
              $this->session->setFlashData('success', 'Sucessfuly created a scheduled class');
              return redirect()->to(base_url('admin/schedsubject'));
            } else {
        ...
        if(in_array($_POST['date'], $holi )){
          ...
          // return redirect()->to(base_url('admin/schedsubject/edit/'.$id));
            ...
            if($schedsubj->edit_schedsubj($id, $_POST)){
              ...
              $this->session->setFlashData('success', 'Sucessfuly edited a scheduled class');
            } else {
              $this->session->setFlashData('error', 'Something went wrong!');
            }
            return redirect()->to(base_url('admin/schedsubject'));
  ...
  public function delete($id){
    ...
    if($schedsubj->inactive($id)) {
          ...
          $this->session->setFlashData('success', 'Successfully deleted a scheduled class');
    } else {
      $this->session->setFlashData('error', 'Something went wrong!');
    }
    return redirect()->to(base_url('admin/schedsubject'));
  ...
  public function active($id){
    ...
    if($schedsubj->active($id)) {
          ...
          $this->session->setFlashData('success', 'Successfully restored a scheduled class');
    } else {
      $this->session->setFlashData('error', 'Something went wrong!');
    }
    return redirect()->to(base_url('admin/schedsubject'));
