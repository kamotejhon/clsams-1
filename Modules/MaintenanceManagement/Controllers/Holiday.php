...
class Holiday extends BaseController {
    ...
    if($this->request->getMethod() === 'post'){
      if($this->validate('holiday')){
        if($holidayModel->add($_POST)){
          ...
          $this->session->setFlashData('success_message', 'Sucessfuly created a holiday');
        } else {
    ...
    if($this->request->getMethod() === 'post'){
      if($this->validate('holiday')){
        if($holidayModel->edit($id, $_POST)){
          ...
          $this->session->setFlashData('success_message', 'Sucessfuly edited a holiday');
        } else {
          $this->session->setFlashData('error_message', 'Something went wrong!');
        }
        return redirect()->to(base_url('admin/holiday'));
  ...
  public function delete($id){
    ...
    if($holidayModel->inactive($id)) {
      ...
      $this->session->setFlashData('success_message', 'Successfully deleted holiday');
    } else {
      $this->session->setFlashData('error_message', 'Something went wrong!');
    }
    return redirect()->to(base_url('admin/holiday'));
  ...
  public function active($id){
    ...
    if($holidayModel->active($id)) {
      ...
      $this->session->setFlashData('success_message', 'Successfully restored holiday');
    } else {
      $this->session->setFlashData('error_message', 'Something went wrong!');
    }
    return redirect()->to(base_url('admin/holiday'));
